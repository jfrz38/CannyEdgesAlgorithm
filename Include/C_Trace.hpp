...
#define C_PrintNum(text,metodo)                           \
{ 							                           \
  C_Trace::Trace (0, __FILE__, __LINE__, text, metodo); \
}

/* Prints a trace with the file name, line number, the desired message and
   a numeric value or variable */
#define C_TraceNum(text,metodo)                           \
{ 							                           \
  C_Trace::Trace ('T', __FILE__, __LINE__, text, metodo); \
}

/* Displays a warning message with the file name, line number, the desired
   message and an int value or variable*/
#define C_WarningNum(text,metodo)                         \
{								                       \
  C_Trace::Trace ('W', __FILE__, __LINE__, text, metodo); \
}

/* Displays a warning message with the file name, line number, the desired
   message and an int value or variable*/
#define C_IfWarningNum(cond,text,metodo)                    \
{								                         \
  if (cond)							                     \
    C_Trace::Trace ('W', __FILE__, __LINE__, text, metodo); \
}
...
#define C_ErrorNum(text,metodo,to_do)                     \
{								                       \
  C_Trace::Trace ('E', __FILE__, __LINE__, text, metodo); \
  to_do;							                   \
}

/* Displays an error message with the file name, line number, the desired
   message and an int value or variable and DOES to_do */
#define C_IfErrorNum(cond,text,metodo,to_do)                \
{								                         \
  if (cond)							                     \
  {								                         \
    C_Trace::Trace ('E', __FILE__, __LINE__, text, metodo); \
